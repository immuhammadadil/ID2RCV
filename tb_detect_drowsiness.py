...
#importing packages for thingsboard
...
#iSetting up things for thingboard and granting access for cloud
ACCESS_TOKEN='KUXHarz5oXKBnH0glKMV'                 #Token of your device
...
client1.loop_start()
while(cap.isOpened()):
	# grab the raw NumPy a# grab the raw NumPy array representing the image, then initialize the timestamp
	...
	if len(rects) > 0:
		...
		cv2.putText(frame1, text, (10, 20), cv2.FONT_HERSHEY_SIMPLEX,
			0.5, (0, 0, 255), 2)
		sensor_data['Driver Position']="Present"
		client1.publish('v1/devices/me/telemetry', json.dumps(sensor_data), 1)
	...
	cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
	if len(rects)==0:
                os.system('espeak "Driver Not in Target Area"')
                cv2.putText(screen, "Face Not Found!", (10, 250),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                sensor_data['Driver Position']="Not Present"
                client1.publish('v1/devices/me/telemetry', json.dumps(sensor_data), 1)
...
#=====================================================================================================
		...
		if(xcord<100)|(xcord>360):
			...
			if Distraction_counter >= Distraction_thresh:
				...
				cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
				#esng.say(‘Drowsiness Detected’)
				os.system('espeak "Distraction Detected"')
				sensor_data['Distraction Status']="Distracted"
				client1.publish('v1/devices/me/telemetry', json.dumps(sensor_data), 1)
				#os.system espeak 'Distraction Detected                    
			...
			Distraction_counter=0
			sensor_data['Distraction Status']="Attentive"
			#payload="{"
                        #payload+="\"Driver's State\":Attentive";
                        #payload+="}"
...
#====================================================================================================			
		...
		if ear < EYE_AR_THRESH:
			COUNTER += 1
			#sensor_data1['Blink Count']=COUNTER
			#client1.publish('v1/devices/me/telemetry', json.dumps(sensor_data1), 1)
			time0=time.clock()
			# if the eyes were closed for a sufficient number of
			...
			if COUNTER >= EYE_AR_CONSEC_FRAMES:
				...
				cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
				sensor_data['Drowsiness Status']="Drowsy"
				client1.publish('v1/devices/me/telemetry', json.dumps(sensor_data), 1)
		...
		# threshold, so reset the counter and alarm
			...
			c=0
			alert_level=5
			sensor_data['Drowsiness Status']="Alert"
			client1.publish('v1/devices/me/telemetry', json.dumps(sensor_data), 1)
